...
public class EntityBlazeTitanMinion extends Blaze implements RangedAttackMob, IMinion {
	...
	protected void dropFewItems(boolean attackedRecently, int loottingLevel) {
		...
		int k;
		for (k = 0; k < j; k++) {
			this.spawnAtLocation(Items.BLAZE_ROD, 1);
		}
		...
		if (this.getRandom().nextInt(60) == 0 || (this.isOnFire() && this.getRandom().nextInt(2) == 0) || this.getRandom().nextInt(1 + loottingLevel) > 0) {
			this.spawnAtLocation(new ItemStack(Blocks.FIRE), 0.0F);
		}
	...
	public boolean canAttackType(EntityType<?> entityType) {
		return entityType != TheTitansNeoEntities.BLAZE_TITAN.get() && entityType != TheTitansNeoEntities.BLAZE_TITAN_MINION.get();
	}
	...
	public boolean hurt(DamageSource damageSource, float amount) {
		...
		if (entity instanceof LivingEntity) {
			...
			for (Entity entity1 : entities) {
				if (entity1 instanceof EntityBlazeTitanMinion) {
					EntityBlazeTitanMinion blazeTitanMinion = (EntityBlazeTitanMinion) entity1;
			...
			if (this.master == null && this.isAlive()) {
				if (!this.level().isClientSide() && this.level().getDifficulty() != Difficulty.PEACEFUL) {
					...
					if (randomRate < TheTitansNeoConfigs.bishopSummonLoyalistSpawnRate.get()) {
						...
						blazeTitanMinion.finalizeSpawn((ServerLevelAccessor) this.level(), this.level().getCurrentDifficultyAt(blazeTitanMinion.blockPosition()), MobSpawnType.SPAWNER, null);

...
public class EntityPigZombieTitanMinion extends ZombifiedPiglin implements RangedAttackMob, IMinion {

	private static final EntityDataAccessor<Integer> MINION_TYPE = SynchedEntityData.defineId(EntityPigZombieTitanMinion.class, EntityDataSerializers.INT);
	...
	public EntityPigZombieTitanMinion(EntityType<? extends EntityPigZombieTitanMinion> entityType, Level level) {
	...
	public EntityPigZombieTitanMinion(Level level) {
		this(TheTitansNeoEntities.PIG_ZOMBIE_TITAN_MINION.get(), level);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new EntityAIFindMaster(this, 100.0D, titan -> titan instanceof EntityPigZombieTitan));
	...
	protected void dropFewItems(boolean attackedRecently, int loottingLevel) {
		...
		for (k = 0; k < j; k++) {
			this.spawnAtLocation(Items.GOLD_NUGGET, 1);
		}
		...
		j = this.getRandom().nextInt(2 + loottingLevel);
		for (k = 0; k < j; k++) {
			this.spawnAtLocation(Items.COOKED_PORKCHOP, 1);
		}
	...
	public Component getName() {
		...
		case PRIEST:
			return Component.translatable("entity.thetitansneo.pig_zombie_priest");
		case ZEALOT:
			return Component.translatable("entity.thetitansneo.pig_zombie_zealot");
		case BISHOP:
			return Component.translatable("entity.thetitansneo.pig_zombie_bishop");
		case TEMPLAR:
			return Component.translatable("entity.thetitansneo.pig_zombie_templar");
		default:
			return Component.translatable("entity.thetitansneo.pig_zombie_loyalist");
		}
	}
	...
	public boolean hurt(DamageSource damageSource, float amount) {
		...
		if (entity instanceof EntityPigZombieTitanMinion || entity instanceof EntityPigZombieTitan) {
			return false;
		}
		...
		if (entity instanceof LivingEntity) {
			...
			for (Entity entity1 : entities) {
				if (entity1 instanceof EntityPigZombieTitanMinion) {
					EntityPigZombieTitanMinion pigZombieTitanMinion = (EntityPigZombieTitanMinion) entity1;
					pigZombieTitanMinion.setTarget(livingEntity);
					pigZombieTitanMinion.randomSoundDelay = this.getRandom().nextInt(40);
				}
	...
	public void performRangedAttack(LivingEntity target, float velocity) {
		...
		if (this.distanceToSqr(target) < (target.getBbWidth() * target.getBbWidth()) + 36.0D) {
			...
			if (randomInt == 0) {
				...
				target.hurt(this.damageSources().starve(), 10.0F);
			...
			if (this.master == null && this.isAlive()) {
				if (!this.level().isClientSide() && this.level().getDifficulty() != Difficulty.PEACEFUL) {
					...
					if (randomRate < TheTitansNeoConfigs.bishopSummonLoyalistSpawnRate.get()) {
						EntityPigZombieTitanMinion pigZombieTitanMinion = new EntityPigZombieTitanMinion(this.level());
						pigZombieTitanMinion.setPos(this.getX(), this.getY(), this.getZ());
						pigZombieTitanMinion.setYRot(this.getYRot());
						pigZombieTitanMinion.finalizeSpawn((ServerLevelAccessor) this.level(), this.level().getCurrentDifficultyAt(pigZombieTitanMinion.blockPosition()), MobSpawnType.SPAWNER, new Zombie.ZombieGroupData(false, true));
						pigZombieTitanMinion.setMinionType(0);
						pigZombieTitanMinion.setBaby(this.isBaby());
						pigZombieTitanMinion.setHealth(pigZombieTitanMinion.getMaxHealth());
						this.level().addFreshEntity(pigZombieTitanMinion);
					}
					...
					if (randomRate < TheTitansNeoConfigs.bishopSummonPriestSpawnRate.get()) {
						...
						pigZombieTitanMinion.setYRot(this.getYRot());
						pigZombieTitanMinion.finalizeSpawn((ServerLevelAccessor) this.level(), this.level().getCurrentDifficultyAt(pigZombieTitanMinion.blockPosition()), MobSpawnType.SPAWNER, new Zombie.ZombieGroupData(false, true));
						pigZombieTitanMinion.setMinionType(1);
						pigZombieTitanMinion.setBaby(this.isBaby());
						pigZombieTitanMinion.setHealth(pigZombieTitanMinion.getMaxHealth());
						this.level().addFreshEntity(pigZombieTitanMinion);
					}
			...
			if (this.master == null && this.getHealth() > 0.0F) {
				if (!this.level().isClientSide() && this.level().getDifficulty() != Difficulty.PEACEFUL) {
					...
					if (randomRate < TheTitansNeoConfigs.templarSummonZealotSpawnRate.get()) {
						...
						pigZombieTitanMinion.setYRot(this.getYRot());
						pigZombieTitanMinion.finalizeSpawn((ServerLevelAccessor) this.level(), this.level().getCurrentDifficultyAt(pigZombieTitanMinion.blockPosition()), MobSpawnType.SPAWNER, new Zombie.ZombieGroupData(false, true));
						pigZombieTitanMinion.setMinionType(2);
						pigZombieTitanMinion.setBaby(this.isBaby());
						pigZombieTitanMinion.setHealth(pigZombieTitanMinion.getMaxHealth());
						this.level().addFreshEntity(pigZombieTitanMinion);
					}
